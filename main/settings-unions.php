...
<?php site_header(); ?>
     ...
     <h3><a href="/main/settings.php">Settings</a>  > <?php echo ($_GET['id'])? '<a href="'.$_SERVER['PHP_SELF'].'">Union Management</a>' : 'Union Management'; ?></h3>
...
<?php if($_GET['id'] > 0 && is_numeric($_GET['id']) || $_POST['submit']) {
	...
	if($_POST['submit']){
		...
		if($affected_rows[0]) { 
			echo '<button class="ui-bar ui-bar-d" data-icon="info" data-theme="d" >Union has been successfully updated!</button>';  
   		} else {
			echo '<button class="ui-bar ui-bar-e" data-icon="info" data-theme="e" >Union update was unsuccessful!</button>';
...
?>
    ...
    <h3 class="ui-bar ui-bar-a">Union Info</h3>
        <div class="ui-grid-a">
       ...
       <form>
        ...
        <table data-role="table" id="union-table" data-filter="true" data-filter-reveal="false" data-input="#filterTable-input" data-mode="reflow" class="ui-responsive ui-employee-table">
            ...
            <tr class="ui-table-thead">
                <th >Edit</th>
                <th data-priority="persist">Union Name</th>
                <th data-priority="1">Weekday Hours</th>
                <th data-priority="2">Saturday</th>
                <th data-priority="3">Sunday</th>
                <th data-priority="4">Holiday</th>
            </tr>
            </thead>
            <?php 
				$queryArgs[0] = "union";
				$unions = $bootstrap->mysqliHandler->getArray($queryArgs); 
				$x=0;
				foreach( $unions as $a_key => $union ) {
